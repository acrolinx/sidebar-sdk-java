...
if [ "$STAGE" = "snapshot" ]; then
    if ./gradlew clean pP publish; then
        exit 0
    else
        exit 1
    fi
...
if [ "$STAGE" = "release" ]; then
        echo "Releasing..."
        if ./gradlew clean pP publish -Psigning.keyId="$keyId" -Psigning.password="$password" -Psigning.secretKeyRingFile="../secring.gpg"; then
            echo "Done with first publish step."
            if is_not_substring "SNAPSHOT" "$PROJECT_VERSION"; then
                echo "ready for release!"
                if ./gradlew clean pP publish closeRepository -Psigning.keyId="$keyId" -Psigning.password="$password" -Psigning.secretKeyRingFile="../secring.gpg"; then
