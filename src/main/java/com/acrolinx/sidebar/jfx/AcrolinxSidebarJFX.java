...
public class AcrolinxSidebarJFX implements AcrolinxSidebar {
  ...
  public AcrolinxSidebarJFX(
    ...
    logger.atInfo().log(
        "Java Version: {} ; UI Framework: Java FX", System.getProperty("java.version"));
    ...
    webEngine.setOnError(webErrorEvent -> logger.error("Error: {}", webErrorEvent));
    webEngine.setOnAlert(webEvent -> logger.debug("Alert: {}", webEvent.getData()));

    webEngine
        .getLoadWorker()
        .stateProperty()
        .addListener(
            (final ObservableValue<? extends Worker.State> observedValue,
                final Worker.State oldState,
                final Worker.State newState) ->
                this.getChangeListener(observedValue, oldState, newState, acrolinxStorage));
    webEngine
        .getLoadWorker()
        .exceptionProperty()
        .addListener(
            (observableValue, oldThrowable, newThrowable) ->
                logger.error("webEngine exception", newThrowable));
  ...
  private void getChangeListener(
      final ObservableValue<? extends Worker.State> observedValue,
      final Worker.State oldState,
      final Worker.State newState,
