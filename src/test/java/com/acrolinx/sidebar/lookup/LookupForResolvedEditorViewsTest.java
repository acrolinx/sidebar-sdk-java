...
class LookupForResolvedEditorViewsTest {
  private static final String TEXT_APPLE =
  ...
  private static final String APPLE_EDITOR_CONTENT = "a apple";

  private static final String TEXT_AUTUMN_FLOWERS =
      "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
  ...
  private static final String AUTUMN_FLOWERS_EDITOR =
      "\n"
          + "Autumn Flowers\n"
  ...
  private static final String REST_DITA_TEXT =
      "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"
  ...
  private static final String REST_DITA_EDITOR =
      "     new data new line starts in div this mastake  test  page  spalling.    tablee         wastee  wastee      wastee  wastee    wastee  wastee    wastee  wastee          tast                           ";
  ...
  void testLookupIgnoreWhiteSpace() {
    ...
    Optional<List<? extends AbstractMatch>> abstractMatches =
        lookup.matchRangesForResolvedEditorView(
            acrolinxMatches,
            TEXT_APPLE,
            APPLE_EDITOR_CONTENT,
            offset ->
                new ContentNode() {
                  @Override
        ...
        .forEach(
            abstractMatch ->
                Assertions.assertEquals(
                    APPLE_EDITOR_CONTENT.substring(
  ...
  void testLookupIgnoreWhiteSpaceIgnoreWhitespaces() {
    ...
    acrolinxMatches.add(
        new AcrolinxMatchWithReplacement("blooming", new IntRange(509, 517), replacement));
    ...
    matchesNew.stream()
        ...
        .forEach(
            abstractMatch ->
                Assertions.assertEquals(
                    AUTUMN_FLOWERS_EDITOR.substring(
                        abstractMatch.getRange().getMinimumInteger(),
                        abstractMatch.getRange().getMaximumInteger()),
                    abstractMatch.getContent()));
    ...
    Assertions.assertEquals(
        "blooming in autumn is", ((AcrolinxMatchWithReplacement) abstractMatch).getReplacement());
  }
  ...
  void testLookupSpecialCases1() {
    ...
    final String getRestLookUpText =
        "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"
            + "<!DOCTYPE glossentry PUBLIC \"-//OASIS//DTD DITA Glossary//EN\" \"glossary.dtd\">\n"
            + "<glossentry id=\"glossentry_s2k_tlv_t1b\">\n"
            + "    <glossterm/>\n"
    ...
    Assertions.assertEquals(
        restLookUpEditor.substring(contentNode.getStartOffset(), contentNode.getEndOffset()),
  ...
  void testLookupIncludesHTMLEntities() {
    final String authorViewContent =
    ...
    abstractMatches.get().stream()
        .forEach(
            abstractMatch ->
                Assertions.assertEquals(
                    authorViewContent.substring(
                        abstractMatch.getRange().getMinimumInteger(),
                        abstractMatch.getRange().getMaximumInteger()),
                    abstractMatch.getContent()));
  }
  ...
  void testLookupOnFileWithRepeatedSameContentAndReferences() {
    ...
    final String authorViewContentBeforeContentNodeString =
        "Instructions for Authors.\n" + " " + matchContent + "\n" + contentNodeString + "\n";
    final String authorViewContent = authorViewContentBeforeContentNodeString + contentNodeString;

    final String documentContentBeforeContentNode =
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
            + "<!DOCTYPE topic PUBLIC \"-//OASIS//DTD DITA Topic//EN\" \"topic.dtd\">\n"
            + "<topic id=\"topic_icd_txy_3db\">\n"
            + "<title>Instructions for Authors.</title>\n"
            + "    <body>\n<p conref=\"mistake-conref\"/>"
            + contentNodeXmlString;
    final String documentContentAfterContentNode = "    </body>\n" + "</topic>\n";
    final String documentContent =
        documentContentBeforeContentNode + contentNodeXmlString + documentContentAfterContentNode;
    ContentNode contentNode =
        new ContentNode() {
          @Override
          public int getStartOffset() {
    ...
    Assertions.assertEquals(
        onlyMatch.getContent(),
        authorViewContent.substring(
            onlyMatch.getRange().getMinimumInteger(), onlyMatch.getRange().getMaximumInteger()));
    Assertions.assertEquals(
        authorViewContentBeforeContentNodeString.length() + 1,
        onlyMatch.getRange().getMinimumInteger());
    Assertions.assertEquals(
        authorViewContentBeforeContentNodeString.length() + 1 + matchContent.length(),
  ...
  void testLookupInReferencedContent() {
    ...
    final String authorViewContent =
        authorViewContentBeforeContentNodeString + contentNodeString + "\n";
    ...
    final String documentContent =
        documentContentBeforeContentNode + referencedContentTag + documentContentAfterContentNode;
    ContentNode contentNode =
        new ExternalContentNode() {
          @Override
    ...
    Assertions.assertEquals(
        onlyMatch.getContent(),
        authorViewContent.substring(
            matchRange.getMinimumInteger(), matchRange.getMaximumInteger()));
    Assertions.assertEquals(
        authorViewContentBeforeContentNodeString.length() + 1, matchRange.getMinimumInteger());
    Assertions.assertEquals(
        authorViewContentBeforeContentNodeString.length() + 1 + matchContent.length(),
    ...
    ContentNode contentNode =
        new ExternalContentNode() {
          ...
          public ReferenceTreeNode getReferenceTree() {
            ...
            ReferenceTreeNode externalContentNode = new ReferenceTreeNode();
            externalContentNode.setResolvedContent(contentNodeResolvedXmlString);
            externalContentNode.setUnresolvedContent(
                "<div>" + secondReferencedContentTag + "</div>");
            externalContentNode.setStartOffsetInParent(0);
            externalContentNode.setReferencingTag(referencedContentTag);

            ReferenceTreeNode externalContentChildNode = new ReferenceTreeNode();
            String firstLevelSubstring =
                contentNodeResolvedXmlString.substring(
                    5, contentNodeResolvedXmlString.length() - 6);
